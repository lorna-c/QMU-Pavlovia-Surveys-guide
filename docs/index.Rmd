...
**If you cannot read the entire document, pay particular attention to the sections on [informed consent](#consent) and [survey settings](#settings).**
Failure to read through these sections at a minimum will lead to broken surveys, poor data quality and even potentially unusable data.

## Latest updates {#updates}

Below are the latest updates to Pavlovia Surveys at the time of writing (`r format(Sys.time(), '%d %B %Y')`).

#### Version choice
You can now select which version of Pavlovia you want to use for your survey - for most purposes, it is recommended to use version 2024.2.0 rather than the latest (2024.3.0) in case of issues with the latest version.  
Be aware that currently, if you have selected v2024.3.0 it reverts to v2024.2.0 when you exit Pavlovia.
If you wish to use v2024.3.0, make sure the survey link you use specifies that version - your link should look something like this: <pre>https://run.pavlovia.org/pavlovia/survey-<b>2024.3.0</b>/?surveyId=08016aae-5160-42e2-869f-7f5402773d93</pre>  
  
Please be aware that if you are using logics with more than one item (e.g. you make a question Visible If {gender} = 2 **and** {group} = 1), in versions 2024.2.0 and 2024.3.0 you need to specify the "block" the questions are in (this is due to the new way they structure surveys in later versions).
In the above example, simply change {gender} = 2 and {group} = 1 to {block_1/gender} = 2 and {block_1/group} = 1 (this assumes your questions are in block 1, which should be the case if you only have one block in your survey - if you have more make sure you specify the right block).

This is also the case for any Expression (read-only) question calling on other questions (e.g. an Expression (read-only) question for scoring might have {q1} + {q2} + {q3} in the Expression box to add up the scores of three questions).
In versions 2024.2.0 and 2024.3.0 you also need to specify the block of questions used in Expression (read-only) questions, like this: {block_1/q1} + {block_1/q2} + {block_1/q3}.

#### Save results on each page
There is now the option to save participant responses on each page (requires using version 2024.3.0).
To enable this, navigate to the Overview tab in your survey on Pavlovia, then find the Settings section and toggle the "Save on each page" option.
This happens in real time, which means a participant's response is saved immediately upon moving on to the next page of a survey.  
As a result, if you download your survey results at various times during data collection, you might get some participants' responses at different stages of completion.
If you are going to use this function, it is recommended to choose a date and time for a final download of your survey responses, and to not work on your data before then.

#### New and improved question types

* additional Single-Line Input question settings: you can now specify what type of response is expected (e.g. Text, Number, Date, etc.)
* additional Single-Select Matrix question settings: you can now choose to require answers for all rows of the question (check the box "Require answer for all rows" under the Validation tab in the question's settings)
* additional Rating scale question settings: you can now choose the scale to be labels (numbers), stars or smiley faces (stars and smileys can be set up to assign different values to each rating point)
...
* More: The more button (three dots ...) contains additional question types
  ...
  + Multiple Textboxes: Creates a form made up of several Single Input questions
...
In this example I will walk you through how to randomly allocate participants to one of two groups (or more), and how to specify what parts of your survey each group should see.

First, make sure you have pages with questions for group 1, and pages with questions for group 2 (if you have more groups, make sure each group is assigned one or more pages).
In this example I will have one page to randomly allocate participants to a group, and one page per group.

In an empty page, before the pages with your questions add an "Expression (read-only)" question and name it ```group```.
In the "Expression" box, paste the following line: ```{group}```.
To ensure your survey saves what group the participant has been assigned to in your final data file, navigate to the Data tab in the question's settings, find the "Clear value if question becomes hidden" dropdown, and select "None".

Now in your pages for group 1, go to the settings and navigate to the "Logic" tab.
Click the wand icon for the "Visible if" box to open the logics window.
Click the "Select" button and choose the ```group``` question, then enter ```1``` in the box under ```group```.

Do the same thing for the group 2 pages, instead entering ```2``` in the box under ```group```.
If you have more groups, continue as above for each group page.

Now your group 1 participants should only see group 1 pages, and similarly for group 2 participants.

To actually assign a group to participants, we will use another online experiment and feed it our survey.
To do this, get your survey's ID from the title banner of your survey in Pavlovia (see picture below).

``` {r, fig.cap = "Survey ID location", fig.alt = "Survey ID location: A screenshot of the Pavlovia Survey designer, and specifically of where to find the Survey ID number.", label = "surveyID", echo = F}
knitr::include_graphics("surveyID.png")
```

Copy the numbers after the # symbol.
Then paste this ID into the following link, replacing the bold text with the ID: <pre>https://run.pavlovia.org/vespr/survey-group/?surveyId=<b>surveyID</b>&groups=2</pre>

Your link should look like this: <pre>https://run.pavlovia.org/vespr/survey-group/?surveyId=ca656b83-b465-4399-a1cf-0ac085f17230&groups=2</pre>

If you have more than 2 groups, change the number behind groups= (at the end of the link) with the number of groups you have (currently tested up to 3 groups).

This new link is the link you should give participants and use on recruitment platforms (e.g. Sona).

Test this link before sending it to participants, making sure the groups are allocated properly (the survey content seen matches the group allocated, the group saved in the data matches the group allocated during the survey, etc.).
Do not distribute your survey until you are certain this works.
...
If you need more control over how many participants are in each group, and don't want to periodically check your data, you can use the method outlined [in the Pavlovia experiments guide](https://lorna-c.github.io/QMU-Pavlovia-Exps-guide/#Conditional_loop_display).
...
Next, you will need to log into Pavlovia and make some changes to your survey (there is a video demonstration after the instructions).
...
Now in your Pavlovia survey (see video demonstration at the end):
